...
import { form, FormField, FormRoot, pattern } from '@angular/forms/signals';
...
@Component({
  ...
  imports: [FormRoot, InputCheckbox, InputText, ErrorAlert],
...
export class WeatherBriefingForm {
  ...
  weatherBriefingForm = form(
    this.weatherBriefingModel,
    (schemaPath) => {
      pattern(schemaPath.airports, /^([A-Z]{4} )*[A-Z]{4}$/, {
        message: 'ICAO airport codes must be 4 uppercase letters separated by spaces',
      });
      pattern(schemaPath.countries, /^([A-Z]{2} )*[A-Z]{2}$/, {
        message: 'WMO country codes must be 2 uppercase letters separated by spaces',
      });
      atLeastOneReportTypeSelected(schemaPath, 'At least 1 Message type must be selected');
      eitherAirportsOrCountriesRequired(
        schemaPath,
        'Enter at least one airport ICAO code or one country WMO code',
      );
    },
    {
      submission: {
